...
- **Sidebar capability approvals** (spec `038`): unlisted Bash commands and external paths outside allowed roots prompt an inline Deny / once / session / always confirmation in the sidebar instead of a modal; always-allow persists the full command or first-token prefix to `bashAllowlist`, or the directory root to device-local `externalReadDirectories`, and session grants clear on session switch.
