...
- Keep the `spawn_agent` schema and system prompt in sync: the schema in `packages/pivi-agent-core/src/engine/pi/createSubagentTool.ts` marks `run_in_background` as required, so the registered-tools prompt in `packages/pivi-agent-core/src/prompt/obsidianAgentTools.ts` must list `run_in_background` in its `Required parameters` sentence. A mismatch causes weaker models to omit the field and burn a turn on tool-validation retries before recovering.
